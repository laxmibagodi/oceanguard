# ============================================================
# OCEANGUARD — Complete Streamlit App v5
# NEW: Image annotation (spill marking) + Always-visible map
# Run: streamlit run app.py
# Install: pip install streamlit tensorflow keras opencv-python
#          folium streamlit-folium pandas numpy pillow matplotlib
# ============================================================
...
def annotate_image(pil_image, label, confidence, probs):
    ...
    if label == "Oil Spill":
        # ── Generate pseudo-heatmap to show WHERE spill likely is ──
        # Convert to grayscale for analysis
        gray = cv2.cvtColor(img_cv, cv2.COLOR_BGR2GRAY)

        # In SAR images, oil spills appear as DARK regions
        # Find darkest regions = likely spill areas
        ...
        for idx, cnt in enumerate(contours[:3]):
            ...
            # Red filled contour (semi-transparent)
            mask_overlay = img_cv.copy()
            cv2.drawContours(mask_overlay, [cnt], -1,
                             (0, 0, 255), -1)
            img_cv = cv2.addWeighted(img_cv, 0.7,
                                     mask_overlay, 0.3, 0)
            ...
            # Label on box
            rank_label = ["PRIMARY", "SECONDARY", "TERTIARY"][idx]
            font_scale = max(0.4, w/1000)
            txt_size   = cv2.getTextSize(
                rank_label, cv2.FONT_HERSHEY_SIMPLEX,
                font_scale, 1)[0]
            cv2.rectangle(img_cv,
                          (x, y-txt_size[1]-8),
                          (x+txt_size[0]+8, y),
                          (0,0,200), -1)
            cv2.putText(img_cv, rank_label,
                        (x+4, y-4),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        font_scale, (255,255,255), 1)

        # ── Top banner ──
        banner_h = max(50, h//10)
        cv2.rectangle(img_cv, (0,0), (w, banner_h),
                      (0,0,180), -1)

        font_scale = max(0.6, w/600)
        cv2.putText(img_cv,
                    f"OIL SPILL DETECTED — {confidence:.1f}% CONFIDENCE",
                    (10, banner_h//2 + 8),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    font_scale, (0,100,255), 2)

        # ── Bottom info bar ──
        bar_y = h - max(35, h//12)
        cv2.rectangle(img_cv, (0, bar_y), (w,h), (0,0,100), -1)
        cv2.putText(img_cv,
                    f"OCEANGUARD AI | Sentinel-1 SAR | "
                    f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
                    (10, bar_y + max(20, h//30)),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    max(0.35, w/1800), (0,200,255), 1)

        # ── Spill count indicator ──
        n_spills = sum(1 for c in contours[:3]
                       if cv2.contourArea(c) > 200)
        cv2.putText(img_cv,
                    f"REGIONS FLAGGED: {n_spills}",
                    (w-max(200, w//4), banner_h//2+8),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    max(0.4, w/1200), (0,200,255), 1)

    else:
        # ── Clean water — green border ──
        border = max(6, w//80)
        cv2.rectangle(img_cv, (0,0), (w,h),
                      (0,200,80), border)
        ...
        cv2.putText(img_cv,
                    ...
                    (10, banner_h//2+8),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    font_scale, (0,255,100), 2)

        bar_y = h - max(35, h//12)
        cv2.rectangle(img_cv,(0,bar_y),(w,h),(0,60,30),-1)
        cv2.putText(img_cv,
...
def build_map(logs=None):
    """Always builds map. Shows detections if any, else shows region markers."""
    m = folium.Map(
        location=[10.0, 75.0],
        zoom_start=3,
    ...
    if not logs:
        # Show all monitored regions as blue dots
        region_group = folium.FeatureGroup(
            name="📡 Monitored Regions").add_to(m)
    ...
    # Has detections — show them
    oil_locs   = [l for l in logs if l['result']=='Oil Spill']
    clean_locs = [l for l in logs if l['result']=='Clean Sea']

    clean_cluster = MarkerCluster(name="🟢 Clean Scans").add_to(m)
    oil_group     = folium.FeatureGroup(
                        name="🔴 Oil Spill Alerts").add_to(m)

    for entry in logs:
        is_oil = entry['result'] == 'Oil Spill'
        ...
        <div style='font-family:monospace;min-width:200px;
            ...
            🕐 {entry['time']}<br>
            📍 {entry['region']}<br>
            🌐 {entry['lat']}°, {entry['lon']}°<br>
            📈 Confidence: {entry['confidence']:.1f}%<br>
            📁 {entry['source'][:28]}
        </div>""", max_width=260)

        if is_oil:
            folium.CircleMarker(
                [entry['lat'], entry['lon']], radius=16,
                color='#ff0000', fill=True,
                fill_color='#ff4444', fill_opacity=0.75, weight=3,
                popup=popup,
                tooltip=f"🚨 {entry['region']} | {entry['confidence']:.1f}%"
            ).add_to(oil_group)
            folium.CircleMarker(
                [entry['lat'], entry['lon']], radius=28,
                color='#ff0000', fill=False, weight=1, opacity=0.3
            ).add_to(oil_group)
        else:
            folium.CircleMarker(
                [entry['lat'], entry['lon']], radius=8,
                color='#00cc66', fill=True,
                fill_color='#00ff88', fill_opacity=0.5, weight=2,
                popup=popup,
                tooltip=f"✅ {entry['region']} | {entry['confidence']:.1f}%"
            ).add_to(clean_cluster)
    ...
    <div style='position:fixed;bottom:30px;left:30px;z-index:1000;
        ...
        <span style='color:#00cc66;font-size:16px;'>●</span> Clean Scan ({len(clean_locs)})<br>
...
# ════════════════════════════════════════════════════════
# TAB 1 — UPLOAD & DETECT (with image annotation)
# ════════════════════════════════════════════════════════
with tab1:
        ...
        with b1:
            if label == "Oil Spill":
                if st.button("📤 Report to Authorities",
                    ...
                    st.success(f"✅ Reported! Location: {region} "
                               f"({lat}, {lon})")
        with b2:
...
with tab2:
    ...
    st.markdown("<p style='color:#7ecfea;'>Real-time detection from "
                "webcam or drone.</p>", unsafe_allow_html=True)

    c1, c2 = st.columns([1,2])
    with c1:
        cam_idx  = st.selectbox("Camera", [0,1,2],
                                 format_func=lambda x: f"Camera {x}")
        n_frames = st.slider("Scan every N frames", 1, 30, 10)
        overlay  = st.checkbox("Show overlay", value=True)
        annotate_live = st.checkbox("Annotate spill regions", value=True)
        go_cam   = st.button("▶️ Start Detection", use_container_width=True)
    ...
    with c2:
        st.markdown("""<div class='ocean-card'>
            <div style='color:#00d4ff;font-weight:600;margin-bottom:0.8rem;'>
                📡 Live Detection Pipeline</div>
            <div style='color:#b0d4e8;font-size:0.85rem;line-height:2;'>
                1. Camera captures frames continuously<br>
                2. Every N frames → AI model prediction<br>
                3. Spill regions marked on frame in red<br>
                4. Detections auto-logged to Map & Analytics
            </div></div>""", unsafe_allow_html=True)
    ...
    if go_cam:
        ...
        if not cap.isOpened():
            ...
            fc = 0; last_label = "Scanning..."; last_conf = 0.0
            t0 = time.time()
            while not stop_cam:
                ret, frame = cap.read()
                if not ret: break
                ...
                if fc % n_frames == 0:
                    ...
                    last_label, last_conf, _ = predict(pil, threshold)
                    lat, lon, region = random_ocean_coord()
                    log_detection(last_label, last_conf,
                                  f"Camera {cam_idx}", lat, lon, region)
                    if annotate_live:
                        rgb = np.array(annotate_image(
                            pil, last_label, last_conf,
                            {"Clean Sea":0,"Oil Spill":0}))
                elif overlay:
                    color = (255,60,60) if last_label=="Oil Spill" \
                            else (0,220,100)
                    cv2.rectangle(rgb,(10,10),(400,70),(0,20,40),-1)
                    cv2.putText(rgb, last_label,
                                (20,45), cv2.FONT_HERSHEY_SIMPLEX,
                                0.9, color, 2)
                    cv2.putText(rgb, f"Confidence: {last_conf:.1f}%",
                                (20,65), cv2.FONT_HERSHEY_SIMPLEX,
                                0.5, (180,220,255), 1)
                frame_ph.image(rgb, use_column_width=True,
                               caption=f"Live — Frame {fc}")
                fps = fc/(time.time()-t0+1e-6)
                fps_ph.markdown(
                    f"<div style='text-align:center;color:#4a8fa8;"
                    f"font-size:0.8rem;'>FPS: {fps:.1f}  |  "
                    f"Frame: {fc}  |  Last: {last_label} "
                    f"({last_conf:.1f}%)</div>",
                    unsafe_allow_html=True)
                time.sleep(0.03)
            cap.release()
        ...
        if not os.path.exists(watch_folder):
            ...
            if not imgs:
                ...
                for i in range(min(max_scans, len(imgs))):
                    ...
                    lat, lon, region = random_ocean_coord()
                    log_detection(label, conf, f"Auto:{f}", lat, lon, region)
                    results.append({"scan":i+1,"file":f,"result":label,
                                    "confidence":conf,
                                    "time":datetime.now().strftime("%H:%M:%S"),
                                    "region":region})

                    display_img = annotate_image(img, label, conf, probs) \
                                  if show_annotated else img
                    img_ph.image(display_img,
                ...
                <div class='ocean-card' style='text-align:center;'>
                    ...
                    <div style='color:#7ecfea;margin-top:0.8rem;'>
                        ...
                        &nbsp;|&nbsp;
                        Clean: <b style='color:#00cc66;'>{len(results)-oil_n}</b>
                    </div></div>""", unsafe_allow_html=True)
...
# ════════════════════════════════════════════════════════
# TAB 4 — LIVE DETECTION MAP (always visible!)
# ════════════════════════════════════════════════════════
with tab4:
    ...
    if not logs:
        # Show info but STILL show the map with monitored regions
        st.markdown("""
        <div class='ocean-card' style='padding:1rem;margin-bottom:1rem;'>
            ...
            <div style='color:#7ecfea;font-size:0.85rem;margin-top:0.3rem;'>
                Blue dots = monitored ocean regions worldwide.
                Run a scan to see oil spill detections appear as
                🔴 red alerts on the map.
            </div>
        </div>""", unsafe_allow_html=True)
    else:
        oil_locs   = [l for l in logs if l['result']=='Oil Spill']
...
with tab5:
    ...
    if not st.session_state.scan_history:
        st.markdown("""<div class='ocean-card' style='text-align:center;padding:2rem;'>
        ...
        ax2.tick_params(colors='#7ecfea')
        ax2.spines[:].set_color('#1a4a6a'); ax2.set_ylim(0,105)
        ...
        if oil_logs:
            ...
            bars3=ax3.barh(regs,cnts,color='#ff4444',alpha=0.85,edgecolor='none')
        ...
        if len(history)>=2:
            ...
            ax4.set_xticks(x)
            ax4.set_xticklabels(hours,rotation=45,color='#7ecfea',fontsize=9)
